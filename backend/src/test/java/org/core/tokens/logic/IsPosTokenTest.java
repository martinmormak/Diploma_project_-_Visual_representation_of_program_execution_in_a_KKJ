...
class IsPosTokenTest {
    ...
    void testStackSolvingValidTokens() {
        List<IToken> tokens1 = new LinkedList<>(List.of(new IntToken(3), new IsPosToken()));
        ...
        List<IToken> tokens2 = new LinkedList<>(List.of(new IntToken(-3), new IsPosToken()));
    ...
    void testStackSolvingWithInvalidTokens() {
        List<IToken> tokens1 = new LinkedList<>(List.of(new BoolToken(true), new IsPosToken()));
        Stack<IToken> stack1 = new Stack<>();
        tokens1.removeFirst().stackSolving(stack1);
        assertEquals(1, stack1.size());
        assertInstanceOf(BoolToken.class, stack1.peek());
    ...
    void testStackSolvingWithInsufficientTokens() {
        List<IToken> tokens1 = new LinkedList<>(List.of(new IsPosToken()));
        Stack<IToken> stack1 = new Stack<>();
