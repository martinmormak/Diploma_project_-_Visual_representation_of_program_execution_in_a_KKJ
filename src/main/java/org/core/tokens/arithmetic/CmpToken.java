...
public class CmpToken implements IFunctionToken {
    private List<IToken> tokens;

    public CmpToken(List<IToken> tokens) {
        this.tokens = tokens;
    }
    ...
    public List<Class<? extends IToken>> getAvailableTokenList() {
        ...
        if(availableTokensFormToken.removeLast() == IArithmeticToken.class && availableTokensFormToken.removeLast() == IArithmeticToken.class) {
            ...
            throw new RuntimeException("Expected two numerals but don't get it for CmpToken");
        }
    }*/
    ...
    public Integer getValue() {
        if (tokens.getLast() instanceof IntToken && tokens.get(tokens.size() - 2) instanceof IntToken) {
            return ((IntToken) tokens.get(tokens.size() - 2)).getValue().compareTo(((IntToken) tokens.getLast()).getValue());
        }
        return null;
    }
    ...
    public List<IToken> termRewritingSolving(List<String> substitutions) {
        ...
        if (tokens.size() >= 2) {
            if (tokens.getLast() instanceof IntToken && tokens.get(tokens.size() - 2) instanceof IntToken) {
                ...
                substitutions.add(tokens.get(tokens.size() - 2) + " " + tokens.getLast() + " CMP -> " + result);
                tokens.removeLast();
                tokens.removeLast();
                tokens.add(result);
                for(IToken token : tokens){
    ...
    public List<IToken> stackSolving(Stack<IToken> stack) {
        ...
        if (stack.size() >= 2) {
            ...
            if (pop1 instanceof IntToken && pop2 instanceof IntToken) {
                IToken result = new IntToken(((IntToken) pop2).getValue().compareTo(((IntToken) pop1).getValue()));
                stack.push(result);
            } else {
    ...
    public String toString() {
        ...
        return stringBuilder.append("CMP").toString();
    }
    ...
    public IFunctionToken clone() {
        ...
        return new CmpToken(tokensClone);
    }
}
