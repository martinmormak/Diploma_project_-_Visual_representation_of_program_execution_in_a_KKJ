...
public class MulToken implements IFunctionToken {
    private List<IToken> tokens;

    public MulToken(List<IToken> tokens) {
        this.tokens = tokens;
    }
    ...
    public List<Class<? extends IToken>> getAvailableTokenList() {
        ...
        if(availableTokensFormToken.removeLast() == IArithmeticToken.class && availableTokensFormToken.removeLast() == IArithmeticToken.class) {
            ...
            throw new RuntimeException("Expected two numerals but don't get it for MulToken");
        }
    }*/
    ...
    public List<IToken> termRewritingSolving(List<String> substitutions) {
        ...
        if (tokens.size() >= 2) {
            if (tokens.getLast() instanceof IntToken && tokens.get(tokens.size() - 2) instanceof IntToken) {
                ...
                substitutions.add(tokens.get(tokens.size() - 2) + " " + tokens.getLast() + " MUL -> " + result);
                tokens.removeLast();
                tokens.removeLast();
                tokens.add(result);
                for(IToken token : tokens){
    ...
    public String toString() {
        ...
        return stringBuilder.append("MUL").toString();
    }
    ...
    public IFunctionToken clone() {
        ...
        return new MulToken(tokensClone);
    }
}
